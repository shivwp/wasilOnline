...
@section('content')
                        <!-- ROW-1 OPEN-->
                            ...
                            <div class="col-md-12 col-lg-12">
                                ...
                                <a href="{{ route('dashboard.testimonials.create') }}" class="btn btn-info-light ">
                                    {{$buton_name}}
                                </a>
                            </div>
                                    <div class="card-body">
                                            ...
                                            <div class="paging-section">
                                                ...
                                                <form>
                                                  ...
                                                  <button type="submit" class="form-control src-btn" ><i class="angle fe fe-search"></i></button>
                                                   <a class="form-control src-btn" href="{{ route('dashboard.testimonials.index') }}"><i class="angle fe fe-rotate-ccw"></i></a>
                                              </div>
                                          </form> 
                                               </div>
                                            <table id="" class="table table-striped table-bordered text-nowrap w-100">
                                                ...
                                                @if(count($testimonials)>0)
                                                    ...
                                                    <tr>
                                                        <td>{{$item->id ?? '' }}</td>
                                                        <td>{{$item->title ?? '' }}</td>
                                                        <td><img src="{{url('testimonials').'/'.$item->image}}" alt="" style="height:50px"></td>
                                                        <td>{{$item->customer_name ?? '' }}</td>
                                                        <td>{{$item->designation ?? '' }}</td>
                                                        <td>
                                                               
                                                                 <a class="btn btn-sm btn-secondary" href="{{ route('dashboard.testimonials.edit', $item->id) }}"><i class="fa fa-edit"></i> </a>
                                                                 
                                                               
                                                                    <form action="{{ route('dashboard.testimonials.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Are you sure');" style="display: inline-block;">
                                        ...
                                        <div id="pagination">{{{ $testimonials->links() }}}</div>
                                    </div>
                                    <!-- TABLE WRAPPER -->
                                </div>
                                <!-- SECTION WRAPPER -->
