  ...
  <div class="card-body" id="add_space">
    <form action="{{ route("dashboard.settings.store") }}" method="post" enctype="multipart/form-data">
      ...
      <div class="row city-shipping" style="display: none">
        ...
        <div class="col-md-12">
          <input type="hidden" id="state_obj" value="{{ json_encode($statedata) }}">
          <div class="row">
            @if(!empty($statedata) || count($statedata) > 0)
              @foreach($statedata as $key => $value)
              <div class="col-md-12 mb-5 state_outer" data-state-index="{{ $key }}">
                <label>{{ $value['state_name'] ?? '' }}</label>
                 <button class="btn btn-primary btn-sm mb-1 float-right add-button" data-index="{{ $key }}" type="button" value="{{ $value['state_id'] ?? '' }}">+</button>
                  <div class="row ">
                      <div class="col-md-12 append-data">
                        @php
                        $privestatecity = App\Models\CityPrice::where('state_id',$value['state_id'])->get();
                        @endphp
                        @if(count($privestatecity) > 0)
                        @foreach($privestatecity as $kkk => $vvv)
                          @if(!empty($vvv) && ($vvv->state_id == $value['state_id']))
                           <div class="row mb-2" data-index="{{ $key }}">
                              <div class="col-md-4">
                                  <select class="form-control" id="state_city" name="selling_city[{{ $key }}][city][]">
                                      <option value="all">All</option>
                                      @foreach($value['city'] as $citykey => $cityval)
                                      <option value="{{ $cityval['city_id'] ?? '' }}" {{isset($vvv) && ($vvv->city_id == $cityval['city_id']) ? 'selected' : ''}}>{{ $cityval['city_name'] ?? '' }}</option>
                                      @endforeach
                                  </select>
                              </div>
                              <input type="hidden" name="selling_city[{{ $key }}][state_id][]" value="{{$value['state_id']}}">
                              <div class="col-md-4">
                                <input type="number" name="selling_city[{{ $key }}][normal_price][]" value="{{!empty($vvv->normal_price) ? $vvv->normal_price : 0}}" class="form-control" id="normal_price" placeholder="normal price">
                              </div>
                               <div class="col-md-4">
                                <input type="number" name="selling_city[{{ $key }}][property_price][]" value="{{!empty($vvv->priority_price) ? $vvv->priority_price : 0}}" class="form-control" id="property_price" placeholder="priority price">
                              </div>
                           </div>
                          @else
                            <div class="row mb-2" data-index="{{ $key }}">
                              <div class="col-md-4">
                                <select class="form-control" id="state_city" name="selling_city[{{ $key }}][city][]">
                                  <option value="all">All</option>
                                  @foreach($value['city'] as $citykey => $cityval)
                                    <option value="{{ $cityval['city_id'] ?? '' }}">{{ $cityval['city_name'] ?? '' }}</option>
                                  @endforeach
                                </select>
                              </div>
                              <input type="hidden" name="selling_city[{{ $key }}][state_id][]" value="{{$value['state_id']}}">
                              <div class="col-md-4">
                                <input type="number" name="selling_city[{{ $key }}][normal_price][]" value="" class="form-control" id="normal_price" placeholder="normal price">
                              </div>
                               <div class="col-md-4">
                                <input type="number" name="selling_city[{{ $key }}][property_price][]" value="" class="form-control" id="property_price" placeholder="priority price">
                              </div>
                            </div>
                          @endif
