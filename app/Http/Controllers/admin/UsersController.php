...

{
    ...
    public function storeTransection($id){
        $d['title'] = 'Store Cradit Transections';
        $UserWalletTransection =UserWalletTransection::select('*')->where('user_id',$id);
        $pagination=10;
        if(isset($_GET['paginate'])){
            $pagination=$_GET['paginate'];
        }
        $d['UserWalletTransection'] = $UserWalletTransection->paginate($pagination)->withQueryString();
        
        return view('admin.users.store-cradit-trans',$d);
    }
